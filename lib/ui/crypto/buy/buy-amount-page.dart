            ...
            buttonWidget(
                onDone: () async {
                  if (cryptoProvider?.cryptoData != null) {
                    // await Provider.of<CryptoProvider>(context, listen: false).getCurrencyDetails(context, "${widget.asset.currency ?? " "}${widget.asset.referenceCurrency ?? " "}");
                    Navigator.push(
                        context,
                        FadeRoute(
                          page: ReviewScreen(
                            ...
                            data: {
                              'amount': displayAmount,
                              'fees': cryptoProvider?.transactionFeesData?.depositFee,
              ...
              child: Padding(
                ...
                child: Column(
                  children: [
                    ...
                    DetailRow(
                      label: 'Total',
                      value: '${widget.asset?.referenceCurrency?.toUpperCase()} ${widget.data['amount']} + ${widget.data['fees']}',
                      isTotal: true,
                    ),
                    const Spacer(),
                    buttonWidget(onDone: () {
