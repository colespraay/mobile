...
class EventProvider extends ChangeNotifier {
  ...
  fetchCategoryListApi(BuildContext context) async {
    ...
    if (apiResponse.error == true) {
      print("fetchCategoryListApi=${apiResponse.errorMessage ?? ""}");
    } else {
  ...
  fetchCreateEventApi(
      BuildContext context, String eventName, String eventDescription, String venue, String eventDate, String time, String category, String eventCoverImage, String longit, String lati) async {
    setloading(true);
    var result = await service.createEvent(eventName, eventDescription, venue, eventDate, time, category, eventCoverImage, mytoken, longit, lati);
    if (result['error'] == true) {
      errorCherryToast(context, result['message']);
    } else {
      eventId = result["id"];
  ...
  fetchUserContactApi(List<Map<String, String?>> user_contacts) async {
    ...
    if (apiResponse.error == true) {
      ...
      // errorCherryToast(apiResponse.errorMessage??"");
    } else {
    ...
    if (result['error'] == true) {
      ...
      if (context.mounted) {
        popupDialog(
            ...
            content: "You have successfully sent ${selectedName[0]} and ${userIds.length - 1} others an invite to your event!",
            buttonTxt: 'Home',
            onTap: () {
            ...
            onTapBtn2: () {
              ...
              Navigator.pushReplacement(
                  ...
                  FadeRoute(
                    ...
                    type: "Spray Gift",
                    date: dateTimeFormat(result['dateCreated'].toString()),
                    amount: '₦$amount',
                    meterNumber: '',
                    transactionRef: '',
  ...
  editEventApi(BuildContext context, String eventName, String eventDescription, String venue, String eventDate, String time, String category, String eventCoverImage, String eventId, String fromPage,
      ...
      if (fromPage == "view_event") {
        if (context.mounted) {
          popupDialog(
              ...
              content: "Your edit has been saved successfully.",
              buttonTxt: 'Home',
              onTap: () {
                Navigator.pop(context);
                Navigator.pop(context);
  ...
  List<UserEventModel>? userHorizontalScrool;
  fetchCurrentUserApi() async {
    setloadingNoNotif(true);
    var apiResponse = await service.currentUserEvents();
    if (apiResponse.error == true) {
      userHorizontalScrool = [];
      userHorizontalScrool = apiResponse.data?.data;
      print("fetchCurrentUserApi=${apiResponse.errorMessage ?? ""}");
      notifyListeners();
      // errorCherryToast(context, apiResponse.errorMessage??"");
    } else {
      userHorizontalScrool = apiResponse.data?.data ?? [];
  ...
  fetchGraphHistoryApi() async {
    ...
    if (apiResponse.error == true) {
      ...
      print("fetchGraphHistoryApi=${apiResponse.errorMessage ?? ""}");
      // errorCherryToast(context, apiResponse.errorMessage??"");
    } else {
